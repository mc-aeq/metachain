...
class SHA3
{
	...
	public:
												SHA3();
												SHA3(HashType type, HashSize size, unsigned int uiDigestLength = 0);
												~SHA3();
		...
		uint256									hash256(HashType type, const uint8_t *pBuffer, unsigned int uiLength, unsigned int uiDigestLength = 0);
		...
		// sponge functionality
		void									reset() { keccakReset(); };
		void									absorb(void *ptr, unsigned int uiSize) { keccakUpdate((uint8_t*)ptr, 0, uiSize); };
		uint256									digest256();
};
