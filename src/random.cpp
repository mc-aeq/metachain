...
{
    LOG_ERROR("Failed to read randomness, aborting", "RAND" );
    abort();
}
...
static void RandAddSeedPerfmon()
{
    ...
    if (ret == ERROR_SUCCESS) {
        ...
        LOG_ERROR( strprintf("%s: %lu bytes\n", __func__, nSize), "RAND" );
    } else {
        static bool warned = false; // Warn only once
        if (!warned) {
			LOG_ERROR(strprintf("%s: Warning: RegQueryValueExA(HKEY_PERFORMANCE_DATA) failed with code %i\n", __func__, ret), "RAND");
            warned = true;
        }
    }
#endif
